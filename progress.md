...
- Installer behavior: local PostgreSQL installs now resolve one explicit live cluster or instance before any admin action, prefer preserving existing Maxogram data over blindly picking the newest version, fail closed instead of silently falling back to port `5432` after discovery errors, and only keep Python interpreters that can create a working `venv` with `pip`.
